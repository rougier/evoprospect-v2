...
# FIGURE 3 : PT PARAMETERS BOXPLOTS
...
ggplot(filtered_data, aes(x = reorder(as.factor(monkey), elo, FUN = mean), y = rho_g)) +  
  ...
  geom_point(
    ...
    size = 1, alpha = 0.6, color = "black", shape = 16
  ) +
  stat_summary(
    geom = "point", 
    fun = median,
    shape = 16, size = 2, fill = "black", color = "red", stroke = 1
  ) +
  labs(
    title = "",
    x = "Monkeys",
  ...
  annotate("text", x = 5, y = 1, label = expression(italic("risk aversion")), color = "black", hjust = 1, size = 8) +
  annotate("text", x = 5, y = -1, label = expression(italic("risk seeking")), color = "black", hjust = 1, size = 8) +
  scale_y_continuous(
...
ggplot(filtered_data, aes(x = reorder(as.factor(monkey), elo, FUN = mean), y = alpha_graw)) +  
  ...
  scale_fill_manual(values = palette) +
  guides(fill = "none", color = "none") +
  annotate("text", x = 7, y = 10, label = expression(italic("S shape")), color = "black", hjust = 1, size = 8) +
  annotate("text", x = 7, y = 0.1, label = expression(italic("inverted S shape")), color = "black", hjust = 1, size = 8) +
  scale_y_log10(
...
ggplot(filtered_data, aes(x = reorder(as.factor(monkey), elo, FUN = mean), y = lambdaraw)) +  
  ...
  geom_hline(yintercept = 1, color = "black", linewidth = 0.5) +
  geom_violin(size = 0.5, bw = 0.1, fill = "red", alpha = 0.5) + 
   geom_point(
    position = position_jitter(width = 0.1, seed = 123), 
    size = 1, alpha = 0.6, color = "black", shape = 16
  ) +
  ...
  scale_fill_manual(values = palette) +
  guides(fill = "none", color = "none") +
  annotate("text", x = 5, y = 10, label = expression(italic("loss aversion")), color = "black", hjust = 1, size = 8) +
  annotate("text", x = 5, y = 0.1, label = expression(italic("loss seeking")), color = "black", hjust = 1, size = 8) +
  scale_y_log10(
    breaks = c(0.1, 0.5, 1, 2, 5, 10),
    limits = c(0.1, 10.1)
  )
