...
rhog_int <- seq(from = -0.16, to = 0.19, by = 0.01)
rhog_coeff <- seq(from = 0.02, to = 0.06, by = 0.005)
...
df_rhol <- data.frame(COP = rep(ratio_mean_seq, length(rhol_int) * length(rhol_coeff)),
                      rho_l_pred = unlist(rho_l_preds),
...
h4 <- lmer(rho_g ~ age_category + sex + COP + mean_trial_number + (1|monkey), data = data_norm)
h5 <- lmer(rho_l ~ age_category + sex + COP + mean_trial_number + (1|monkey), data = data_norm)
...
alphag_int <- seq(from = -0.27, to = 0.42, by = 0.01)
alphag_coeff <- seq(from = -0.07, to = -0.02, by = 0.005)
...
h4 <- lmer(alpha_g ~ age_category + sex + COP + mean_trial_number + (1|monkey), data = data_norm)
h5 <- lmer(alpha_l ~ age_category + sex + COP + mean_trial_number + (1|monkey), data = data_norm)
